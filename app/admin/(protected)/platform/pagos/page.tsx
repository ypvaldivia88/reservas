...
import CertificateIssuedDialog, {
  IssuedCertificate,
} from "@/components/platform/CertificateIssuedDialog";
import {
  CompactMetricRow,
  MetricDashboardCard,
  SegmentedControl,
...
export default function PlatformPagosPage() {
  ...
  const handleAction = async (paymentId: string, action: "approve" | "reject") => {
    ...
    const data = await res.json();
    if (data.success) {
      if (action === "approve" && data.data?.certificate) {
        const cert = data.data.certificate;
        setIssuedCertificate({
          code: cert.code,
          salonNombre: cert.salonNombre,
          planNombre: cert.planNombre,
          ciclo: cert.ciclo as BillingCycle,
          adminPhone: cert.adminPhone,
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Aprueba pagos y genera certificados de activación para los salones
          </p>
        </div>
