...
interface ReservasTableProps {
  ...
  onUpdateStatus: (reserva: Reserva, estado: Reserva["estado"], openWhatsApp?: boolean) => void;
}
...
export default function ReservasTable({
  ...
  const getFilteredReservas = () => {
    if (viewMode === "month") {
      ...
      return selectedDate && reservasPorFecha[selectedDate] 
        ? reservasPorFecha[selectedDate].sort((a, b) => a.horaCita.localeCompare(b.horaCita))
        : [];
    } else {
      // En vista semana, mostrar reservas de toda la semana actual
      const weekDates = getWeekDates();
      return weekDates
        .filter(date => reservasPorFecha[date])
  ...
  const fechasOrdenadas = viewMode === "week" 
    ? getWeekDates().filter(date => reservasPorFecha[date])
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800/30 rounded-xl border-2 border-gray-200 dark:border-gray-700 p-4 max-w-md mx-auto">
          ...
          <div className="grid grid-cols-7 gap-1 mb-2">
            {dayNames.map((day, index) => (
              <div key={index} className="text-center text-xs font-bold text-gray-500 dark:text-gray-400 py-2">
                {day}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-1.5">
            {getMonthCalendar().map((date, index) => {
              ...
              const reservasCount = hasReservas ? reservasPorFecha[dateStr].length : 0;
                ...
                >
                  ...
                  {hasReservas && isCurrentMonth && (
                    <div className="absolute bottom-0.5 sm:bottom-1 left-1/2 transform -translate-x-1/2 flex gap-0.5">
                      {Array.from({ length: Math.min(reservasCount, 3) }).map((_, i) => (
                        <div
                          key={i}
                          className={`w-1 h-1 rounded-full ${
                            isSelected || isToday ? "bg-white" : "bg-blue-500 dark:bg-blue-400"
                          }`}
                        />
                      ))}
                    </div>
      ...
      {/* Lista de reservas */}
      <div className="space-y-6">
      {viewMode === "month" && selectedDate && !reservasPorFecha[selectedDate] && (
      ...
      {viewMode === "week" && fechasOrdenadas.length === 0 && (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
          <svg className="w-12 h-12 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <p className="text-sm">No hay reservas para esta semana</p>
        </div>
      )}

      {fechasOrdenadas.map((fecha) => {
        const reservasDelDia = reservasPorFecha[fecha].sort((a, b) => {
          return a.horaCita.localeCompare(b.horaCita);
        });

        return (
          <div key={fecha}>
            {/* Header de fecha (solo mostrar en vista semana) */}
            {viewMode === "week" && (
                        ...
                        <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 truncate">
                              <span className="capitalize">{reserva.forma}</span>
                              {" • "}
                              <span>Largo {reserva.largo}</span>
                              {reserva.telefono && (
                                <>
                                  {" • "}
                                  <span className="hidden sm:inline">{reserva.telefono}</span>
                                </>
                              )}
                            </div>
                            {reserva.decoracion && (
