...
export default function MetricDashboardCard({
  ...
  const hasClickableDetails = details?.some((item) => item.onClick) ?? false;
  const useWholeCardButton = Boolean(onClick) && !hasClickableDetails;
  ...
  const footerEl = footer ? (
    hasClickableDetails && onClick ? (
      <button
        type="button"
        onClick={onClick}
        className="mt-4 w-full text-left text-xs text-muted-foreground transition-colors hover:text-foreground"
      >
        {footer}
      </button>
    ) : (
  ...
  return (
    <DashboardCard
      as={useWholeCardButton ? "button" : "article"}
      onClick={useWholeCardButton ? onClick : undefined}
      ...
      {hasClickableDetails && onClick ? (
        <>
          <button
            type="button"
            onClick={onClick}
            className="w-full rounded-lg text-left transition-colors hover:bg-muted/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
            {header}
            {valueRow}
          </button>
